...
foreach ($commande[$mail] as $id_cmd => $details) {
    if (isset($_REQUEST[aff_num_cmd($details["num"])])) {
        ...
        file_put_contents(
            ...
            json_encode($panier, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE),
        );
...
<body>
    ...
    <section class="contenu-profil">
     ...
     aff_temps($details["date"]["minute"]); ?>)</strong>
                                    <form method="POST"><button name="<?php aff_num_cmd_ou_fidelite(
                                        $details["num"],
                                        1,
                                    ); ?>" class="bouton-recommande">Recommander</button></form>
                                </div>
                                <?php foreach (
                                    $details["plats"]
                                    as $produit
                        ...
                        <div class="commande">
                            <div class="numero">
                                ...
                                <button name="<?php aff_num_cmd_ou_fidelite(
                                    $details["num"],
                                    1,
                                ); ?>" class="bouton-recommande">Commander</button>
                            </div>
                        </div>
                <?php
                }
