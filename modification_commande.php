...
if (isset($_POST['valider_modification_negative'])) {
    ...
    if (empty($cmd_temp[$id_cmd_actuelle]["plats"]) && empty($cmd_temp[$id_cmd_actuelle]["menus"])) {
...
if (!isset($_SESSION["montant_initial_" . $id_cmd_actuelle])) {
    if ($commande[$id_cmd_actuelle]["reduction"] == true) {
        ...
        $_SESSION["montant_initial_" . $id_cmd_actuelle] = $commande[$id_cmd_actuelle]["total"];
    }
}
...
if (isset($ma_commande["plats"])) {
    foreach ($ma_commande["plats"] as $id => $detail) {
        if (isset($_REQUEST["btn_suppr_" . str_replace(" ", "_", $id)])) {
            $cmd_total[$id_cmd_actuelle]["total"] = round($cmd_total[$id_cmd_actuelle]["total"] - ($detail["prix"] * $detail["quantite"]), 2);
            unset($cmd_total[$id_cmd_actuelle]["plats"][$id]);
            file_put_contents("donnees/commande.json", json_encode($cmd_total, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
            header("Location: modification_commande.php?cmd=" . $id_cmd_actuelle);
            exit();
        } 
        elseif (isset($_REQUEST["btn_plus_" . str_replace(" ", "_", $id)])) {
            $cmd_total[$id_cmd_actuelle]["total"] = round($cmd_total[$id_cmd_actuelle]["total"] + $detail["prix"], 2);
...
if (isset($ma_commande["menus"])) {
    foreach ($ma_commande["menus"] as $id_m => $detail_m) {
        if (isset($_REQUEST["btn_suppr_" . str_replace(" ", "_", $id_m)])) {
            $cmd_total[$id_cmd_actuelle]["total"] = round($cmd_total[$id_cmd_actuelle]["total"] - ($detail_m["prix"] * $detail_m["quantite"]), 2);
        ...
        elseif (isset($_REQUEST["btn_plus_" . str_replace(" ", "_", $id_m)])) {
            $cmd_total[$id_cmd_actuelle]["total"] = round($cmd_total[$id_cmd_actuelle]["total"] + $detail_m["prix"], 2);
...
if(isset($_REQUEST['mode_temps'])){
    ...
    $cmd_total[$id_cmd_actuelle]['date_voulue'] = ($cmd_total[$id_cmd_actuelle]['planification'] == 'planifie') ? $_REQUEST['date_prevue'] : 'maintenant';
    file_put_contents("donnees/commande.json", json_encode($cmd_total, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
}
...
if ($ma_commande["reduction"] == true) {
    ...
    $montant = number_format((3 * $cmd_total[$id_cmd_actuelle]["total"]) / 4, 2, ".", "");
} 
else {
    $reduc = 0;
            ...
            <div class="recapitulatif">
                ...
                <?php } else { ?>
                    <form action="modification_commande.php?cmd=<?php echo $id_cmd_actuelle; ?>" method="POST" class="form-modification">
                        <div class="bloc-planification">
                            <p><strong>💰 Compensation pour le retrait (<?php echo number_format(abs($difference), 2, ',', ' '); ?>€) :</strong></p>
                            ...
                            <input type="hidden" name="montant_diff_abs" value="<?php echo abs($difference); ?>">
                        </div>
                <?php } ?>
                ...
                <?php if ($difference > 0) { ?>
                    <form action='https://www.plateforme-smc.fr/cybank/index.php' method='POST' class="form-modification">
                        ...
                        <input class="bouton-recommande" type='submit' value="Payer le surplus (+<?php echo number_format($difference, 2, ',', ' '); ?>€)">
                    </form>
                <?php } else { ?>
